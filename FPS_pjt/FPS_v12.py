...
pygame.display.set_caption("Online Tactical Shooter - Chat & Reload Patch")
...
class GameState:
    def __init__(self):
        ...
        self.m_obs = []
        self.effects = []
        self.chat = [] # [추가] 채팅 로그 리스트

is_host = False
my_socket = None
g_data = GameState()
my_inputs = {'up':0, 'down':0, 'left':0, 'right':0, 'dash':0, 'shoot':0, 'reload':0, 'swap':0, 'mx':0, 'my':0, 'c_idx':0, 'ready':False, 'chat':None}
...
# [패치] 한글 채팅을 위해 폰트를 인자로 받도록 수정
def draw_txt(txt, size, col, x, y, center=True, font_name="impact"):
...
def host_server():
    ...
    try: server.bind(('', PORT))
        ...
        g_data.phase = "MENU"; return
    server.listen(1)
    server.settimeout(0.5)
    
    conn = None
    while g_data.phase == "HOSTING":
        try:
    ...
    if conn and g_data.phase != "MENU":
        conn_file = conn.makefile('rb')
        while g_data.phase != "MENU":
            try:
                p2_inputs = pickle.load(conn_file)
                conn.sendall(pickle.dumps(g_data))
                g_data.effects.clear()
            except: break
        conn.close()
    server.close()
    if g_data.phase != "MENU": g_data.phase = "MENU"
...
class SPlayer:
    ...
    def update(self, inp, bullets):
        ...
        self.d['mx'], self.d['my'] = inp.get('mx', self.d['x']), inp.get('my', self.d['y'])
        
        dx_m, dy_m = self.d['mx'] - self.d['x'], self.d['my'] - self.d['y']
        mag_m = math.hypot(dx_m, dy_m) or 1
        self.d['dir'] = (dx_m/mag_m, dy_m/mag_m)

        if inp.get('swap') and not self.swap_p:
            self.d['use_g'] = not self.d['use_g']
            self.swap_p = True
        elif not inp.get('swap'): self.swap_p = False

        spd = CHAR_DATA[self.d['c_idx']]['speed'] * (1.5 if self.spd_buff>0 else 1.0)
        if inp.get('dash') and self.dash_cd == 0 and not self.dash_p:
            self.dash_cd = 10; self.dash_p = True
        elif not inp.get('dash'): self.dash_p = False
        ...
        mx, my = 0, 0
        if inp.get('up'): my -= 1
        if inp.get('down'): my += 1
        if inp.get('left'): mx -= 1
        if inp.get('right'): mx += 1
        ...
        if inp.get('reload') and self.d['reload']==0 and w['mag']!=-1 and self.d['ammo']<w['mag']:
            self.d['reload'] = w['rl']; self.d['ammo'] = 0

        if inp.get('shoot') and self.timer<=0 and self.d['reload']==0:
            if self.d['use_g'] and self.d['grenades']>0:
                bullets.append({"x":self.d['x'], "y":self.d['y'], "dx":self.d['dir'][0], "dy":self.d['dir'][1], "t":"grenade", "life":60, "own":id(self), "spd":8})
                ...
                bullets.append({"x":self.d['x'], "y":self.d['y'], "dx":self.d['dir'][0], "dy":self.d['dir'][1], "t":w['type'], "life":life_time, "own":id(self), "dmg":w['dmg'], "spd":w['spd']})
                if w['type']=="melee":
...
while True:
    ...
    # [패치] 채팅 중(is_typing)일 때는 WASD, 대시, 사격, 장전을 무시합니다! (R키 수동 재장전 추가)
    my_inputs = {
        'up': keys[pygame.K_w] and not is_typing, 
        'down': keys[pygame.K_s] and not is_typing, 
        'left': keys[pygame.K_a] and not is_typing, 
        'right': keys[pygame.K_d] and not is_typing, 
        'dash': keys[pygame.K_SPACE] and not is_typing, 
        'swap': keys[pygame.K_q] and not is_typing, 
        'shoot': m_btns[0] and not is_typing, 
        'reload': (m_btns[2] or keys[pygame.K_r]) and not is_typing, 
        'mx': mx, 'my': my,
        'c_idx': client_c_idx, 'ready': client_ready, 
        'chat': pending_chat
    }
    pending_chat = None # 한번 보내면 즉시 초기화
    ...
    for e in pygame.event.get():
        ...
        if e.type == pygame.KEYDOWN:
            # [추가] 인게임 채팅 시스템
            if g_data.phase in ["PLAYING", "GAME_OVER"]:
            ...
            if not is_typing:
                if g_data.phase == "MENU":
                    ...
                    if is_host:
                        if not g_data.p1['ready']:
                            if e.key == pygame.K_a: g_data.p1['c_idx'] = (g_data.p1['c_idx'] - 1) % 4
                            elif e.key == pygame.K_d: g_data.p1['c_idx'] = (g_data.p1['c_idx'] + 1) % 4
                            elif e.key == pygame.K_RETURN: g_data.p1['ready'] = True
                        elif e.key == pygame.K_BACKSPACE: g_data.p1['ready'] = False
                    else:
                        if not client_ready:
                            if e.key == pygame.K_a: client_c_idx = (client_c_idx - 1) % 4
                            elif e.key == pygame.K_d: client_c_idx = (client_c_idx + 1) % 4
                            elif e.key == pygame.K_RETURN: client_ready = True
                        elif e.key == pygame.K_BACKSPACE: client_ready = False
                elif g_data.phase == "GAME_OVER" and is_host and e.key == pygame.K_r:
                    g_data.p1['ready'] = False; g_data.p2['ready'] = False; client_ready = False
                    g_data.p1['c_idx'] = 0; g_data.p2['c_idx'] = 0; client_c_idx = 0
                    g_data.phase = "SELECT_MAP"

    if is_host and g_data.phase == "CHAR_SELECT":
        g_data.p2['c_idx'] = p2_inputs.get('c_idx', 0)
        g_data.p2['ready'] = p2_inputs.get('ready', False)

    if is_host and g_data.phase in ["PLAYING", "GAME_OVER"]:
        # [패치] 서버에서 채팅 메시지를 수신하고 관리합니다.
        if my_inputs.get('chat'):
            g_data.chat.append({"txt": f"P1: {my_inputs['chat']}", "life": 300})
            my_inputs['chat'] = None
        if p2_inputs.get('chat'):
            g_data.chat.append({"txt": f"P2: {p2_inputs['chat']}", "life": 300})
            p2_inputs['chat'] = None
    ...
    if is_host and g_data.phase == "PLAYING":
        ...
        for b in g_data.bullets[:]:
            ...
            if b['life'] <= 0 or b['x']<0 or b['x']>WIDTH or b['y']<0 or b['y']>HEIGHT:
                if b['t'] in ["grenade", "bazooka"]:
                    ...
                    for p, pr in [(g_data.p1, p1r), (g_data.p2, p2r)]:
                        if math.hypot(p['x']-b['x'], p['y']-b['y']) < 80: p['hp'] -= (80 if b['t']=="bazooka" else 60)
            ...
            for p_name, p, pr, col in [("p1", g_data.p1, p1r, RED), ("p2", g_data.p2, p2r, BLUE)]:
                target_id = id(sp1) if p_name == "p1" else id(sp2)
                if b['own'] != target_id:
                    if br.colliderect(pr):
                        if b['t'] in ["grenade", "bazooka"]:
                            ...
                            for pp in [g_data.p1, g_data.p2]:
                                if math.hypot(pp['x']-b['x'], pp['y']-b['y']) < 80: pp['hp'] -= (80 if b['t']=="bazooka" else 60)
                        else:
                            dmg = b['dmg']
                            if p['shield'] >= dmg: p['shield'] -= dmg
                            else: dmg -= p['shield']; p['shield'] = 0; p['hp'] -= dmg
                            trigger_effect(["hit", b['x'], b['y'], col])
                        if b in g_data.bullets: g_data.bullets.remove(b)
    ...
    if is_host and g_data.phase == "CHAR_SELECT" and g_data.p1['ready'] and g_data.p2['ready']:
        for p in [g_data.p1, g_data.p2]:
            if p['c_idx'] == 3:
                p['c_idx'] = 4 if random.random() < 0.1 else random.randint(0, 2)
    ...
    if g_data.phase == "MENU":
    ...
    elif g_data.phase == "HOSTING": 
        draw_txt(f"Waiting for player... Tell IP: {get_ip()}", 30, YELLOW, WIDTH//2, HEIGHT//2)
        draw_txt("Press ESC to Cancel", 20, GRAY, WIDTH//2, HEIGHT//2+50)
    elif g_data.phase == "JOINING":
        ...
        for c in g_data.chat:
            draw_txt(c['txt'], 16, WHITE, 20, chat_y, False, font_name="malgungothic, arial")
            chat_y += 20
            
        if is_typing:
            pygame.draw.rect(screen, (50, 50, 50), (20, HEIGHT - 110, 300, 25))
            pygame.draw.rect(screen, WHITE, (20, HEIGHT - 110, 300, 25), 1)
            draw_txt("Say: " + chat_txt + "_", 16, YELLOW, 25, HEIGHT - 105, False, font_name="malgungothic, arial")
        ...
        if g_data.phase == "GAME_OVER":
            ...
            if is_host and not is_typing: draw_txt("Press 'R' to return to Map Select", 30, WHITE, WIDTH//2, HEIGHT//2+60)
