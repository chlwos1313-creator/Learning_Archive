    ...
    for e in pygame.event.get():
        ...
        if e.type == pygame.KEYDOWN:
            if g_data.phase == "MENU":
                ...
                if is_host: # 방장(Host)은 서버 데이터를 직접 수정
                ...
                else: # 손님(Client)은 서버로 보낼 내 상태(my_inputs)만 수정
                    if not my_inputs['ready']:
                        if e.key == pygame.K_a: my_inputs['c_idx'] = (my_inputs['c_idx'] - 1) % 3
                        elif e.key == pygame.K_d: my_inputs['c_idx'] = (my_inputs['c_idx'] + 1) % 3
                        elif e.key == pygame.K_RETURN: my_inputs['ready'] = True
                    elif e.key == pygame.K_BACKSPACE: my_inputs['ready'] = False
