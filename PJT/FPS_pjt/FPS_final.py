...
pygame.display.set_caption("Online Tactical Shooter - Weapon Swap & Korean Patch")
...
def create_player(team, name="Player"):
    return {'team': team, 'name': name, 'c_idx': 0, 'ready': False, 'hp': 100, 'shield': 0, 'x': -100, 'y': -100,
            'ammo': 0, 'grenades': 0, 'reload': 0, 'use_g': False, 'mx': 0, 'my': 0, 'dir': (1,0),
            'timer': 0, 'dash_cd': 0, 'spd_buff': 0, 'swap_p': False, 'dash_p': False,
            'last_t': 0, 'last_a': 0, 'last_d': 0, 'last_rdy': 0, 'last_m': 0,
            'kills': 0, 'deaths': 0, 'dmg_dealt': 0, 'score': 0}

class GameState:
    def __init__(self):
        self.phase = "MENU"
        self.mode = "TEAM" # "TEAM" 또는 "FFA"
        self.map_idx = 0
        self.round_num = 1
        self.red_score = 0
        self.blue_score = 0
...
my_inputs = {'up':0, 'down':0, 'left':0, 'right':0, 'dash':0, 'shoot':0, 'reload':0, 'swap':0, 'mx':0, 'my':0, 
             'chat':None, 'a_t':0, 'a_a':0, 'a_d':0, 'a_rdy':0, 'a_m':0, 'name':"", 'w1':0, 'w2':0, 'w3':0}
...
def trigger_effect(eff):
    ...
    if is_host:
        if eff[0]=="shake": add_shake(eff[1])
        elif eff[0]=="explode": 
            for _ in range(30): particles.append(Particle(eff[1], eff[2], ORANGE, 5, 30))
        elif eff[0]=="hit":
            for _ in range(5): particles.append(Particle(eff[1], eff[2], eff[3], 4, 20))
...
def handle_client(conn, pid):
    ...
    if pid in server_inputs: del server_inputs[pid]
    conn.close()
...
def join_server(ip):
    ...
    try:
        ...
        while g_data.phase != "MENU":
            ...
            if my_inputs.get('chat'): my_inputs['chat'] = None
            g_data = pickle.load(sock_file)
            
            for eid, e in list(g_data.effects):
...
def srv_load_map(idx):
    ...
    idx = idx % 5
    g_data.bullets.clear(); g_data.items.clear(); g_data.chat.clear()
...
while True:
    ...
    my_inputs.update({
        ...
        'w1': keys[pygame.K_1] and not is_typing, 'w2': keys[pygame.K_2] and not is_typing, 'w3': keys[pygame.K_3] and not is_typing,
        'mx': mx, 'my': my, 'name': my_name if my_name.strip() else "Player"
    })
        ...
        if e.type == pygame.KEYDOWN:
            if g_data.phase in ["LOBBY", "PLAYING", "ROUND_OVER", "MATCH_OVER"]:
                if e.key == pygame.K_RETURN:
                    if is_typing:
                        if chat_txt.strip(): my_inputs['chat'] = chat_txt
                    ...
                    if e.key == pygame.K_ESCAPE: is_typing = False; chat_txt = ""
                        ...
                        try: pygame.scrap.init(); txt = pygame.scrap.get(pygame.SCRAP_TEXT); chat_txt += txt.decode('utf-8').replace('\0', '') if txt else ""
                        except: pass
            ...
            if not is_typing:
                ...
                if e.key == pygame.K_m: my_inputs['a_m'] += 1 
                if e.key == pygame.K_SPACE: my_inputs['a_rdy'] += 1
                
                if e.key == pygame.K_ESCAPE and g_data.phase in ["HOSTING", "JOINING"]:
                ...
                if g_data.phase == "MENU":
                    if e.key == pygame.K_1: 
                        ...
                        g_data.players[0] = create_player(0, my_name if my_name.strip() else "Host")
                        threading.Thread(target=accept_clients, daemon=True).start()
                        g_data.phase = "LOBBY"
                    elif e.key == pygame.K_2: g_data.phase = "JOINING"
                    elif e.key == pygame.K_BACKSPACE: my_name = my_name[:-1]
                    elif e.key == pygame.K_v and (pygame.key.get_mods() & pygame.KMOD_CTRL):
                        try: pygame.scrap.init(); txt = pygame.scrap.get(pygame.SCRAP_TEXT); my_name += txt.decode('utf-8').replace('\0', '')[:10] if txt else ""
                        except: pass
                    elif len(my_name) < 10 and e.key not in [pygame.K_RETURN, pygame.K_ESCAPE]:
                        my_name += e.unicode
                    ...
                    if e.key == pygame.K_RETURN: threading.Thread(target=join_server, args=(ip_inp,), daemon=True).start()
                        ...
                        try: pygame.scrap.init(); txt = pygame.scrap.get(pygame.SCRAP_TEXT); ip_inp += txt.decode('utf-8').replace('\0', '') if txt else ""
                        except: pass
    ...
    if is_host:
        ...
        for pid, inp in list(server_inputs.items()):
            if pid in g_data.players and 'name' in inp:
                g_data.players[pid]['name'] = inp['name']
                
            if inp.get('chat'):
                p_name = g_data.players.get(pid, {}).get('name', f"Player {pid}")
                g_data.chat.append({"txt": f"[{p_name}]: {inp['chat']}", "life": 300})
                inp['chat'] = None
        ...
        if g_data.phase == "LOBBY":
            ...
            for pid, p in list(g_data.players.items()):
                ...
                if inp.get('a_t', 0) > p.get('last_t', 0): p['team'] = 1 - p['team']; p['last_t'] = inp['a_t']
                if inp.get('a_a', 0) > p.get('last_a', 0): p['c_idx'] = (p['c_idx'] - 1) % 4; p['last_a'] = inp['a_a']
                if inp.get('a_d', 0) > p.get('last_d', 0): p['c_idx'] = (p['c_idx'] + 1) % 4; p['last_d'] = inp['a_d']
                
                if inp.get('a_m', 0) > p.get('last_m', 0):
                    if pid == 0: g_data.mode = "FFA" if getattr(g_data, 'mode', 'TEAM') == "TEAM" else "TEAM"
                    p['last_m'] = inp['a_m']
                    
                if inp.get('a_rdy', 0) > p.get('last_rdy', 0): p['ready'] = not p.get('ready', False); p['last_rdy'] = inp['a_rdy']
                if not p.get('ready', False): all_ready = False
                
            if all_ready:
                g_data.red_score, g_data.blue_score, g_data.round_num = 0, 0, 1
                g_data.map_idx = 0
                for ptid, pt in g_data.players.items():
                    pt['kills'] = 0; pt['deaths'] = 0; pt['dmg_dealt'] = 0; pt['score'] = 0
                    if g_data.mode == "FFA": pt['team'] = ptid 
                srv_load_map(g_data.map_idx)
                g_data.msg = "MATCH START!" if g_data.mode == "FFA" else "ROUND 1 START!"
                g_data.phase = "PLAYING"

        elif g_data.phase == "PLAYING":
            for pid, p in list(g_data.players.items()):
                if p.get('hp', 0) <= 0: continue
                inp = server_inputs.get(pid, {})
                
                new_wep = p['c_idx']
                if inp.get('w1'): new_wep = 0 
                elif inp.get('w2'): new_wep = 1 
                elif inp.get('w3'): new_wep = 2 
                
                if new_wep != p['c_idx'] and new_wep in [0, 1, 2]:
                    p['c_idx'] = new_wep
                    p['ammo'] = CHAR_DATA[new_wep]['wp']['mag']
                    p['reload'] = 0
                    p['timer'] = 20 
                ...
                for ob in g_data.obs + g_data.m_obs:
                    if rect.colliderect(pygame.Rect(ob)): p['x'], p['y'] = old_x, old_y
            ...
            for b in g_data.bullets[:]:
                ...
                if not should_explode:
                    for pid, p in list(g_data.players.items()):
                        if p.get('hp', 0)>0 and b['team'] != p.get('team') and br.colliderect(pygame.Rect(p['x']-16, p['y']-16, 32, 32)):
                            if b['t'] not in ["grenade", "bazooka"]:
                                dmg = b.get('dmg', 0)
                                if b['own'] in g_data.players: g_data.players[b['own']]['dmg_dealt'] += dmg
                                ...
                                if p['hp'] <= 0:
                                    p['deaths'] += 1
                                    if b['own'] in g_data.players and b['own'] != pid:
                                        g_data.players[b['own']]['kills'] += 1
                                        g_data.players[b['own']]['score'] += 1
                                    if g_data.mode == "FFA":
                                        p['hp'], p['shield'] = 100, 0
                                        p['ammo'] = CHAR_DATA[p['c_idx']]['wp']['mag']
                                        p['x'], p['y'] = random.choice(red_spawns + blue_spawns)
                ...
                if should_explode:
                    if b['t'] in ["grenade", "bazooka"]:
                        ...
                        for ptid, pt in list(g_data.players.items()):
                            if pt.get('hp', 0) > 0 and pt.get('team') != b['team']:
                                if math.hypot(pt['x']-b['x'], pt['y']-b['y']) < 80: 
                                    dmg = 80 if b['t']=="bazooka" else 60
                                    if b['own'] in g_data.players: g_data.players[b['own']]['dmg_dealt'] += dmg
                                    if pt['shield'] >= dmg: pt['shield'] -= dmg
                                    else: dmg -= pt['shield']; pt['shield'] = 0; pt['hp'] -= dmg
                                    
                                    if pt['hp'] <= 0:
                                        pt['deaths'] += 1
                                        if b['own'] in g_data.players and b['own'] != ptid:
                                            g_data.players[b['own']]['kills'] += 1
                                            g_data.players[b['own']]['score'] += 1
                                        if getattr(g_data, 'mode', 'TEAM') == "FFA":
                                            pt['hp'], pt['shield'] = 100, 0
                                            pt['ammo'] = CHAR_DATA[pt['c_idx']]['wp']['mag']
                                            pt['x'], pt['y'] = random.choice(red_spawns + blue_spawns)
                    if b in g_data.bullets: g_data.bullets.remove(b)

            if getattr(g_data, 'mode', 'TEAM') == "TEAM":
                red_alive = sum(1 for p in g_data.players.values() if p.get('team')==0 and p.get('hp', 0)>0)
                blue_alive = sum(1 for p in g_data.players.values() if p.get('team')==1 and p.get('hp', 0)>0)
                ...
                if red_alive == 0 or blue_alive == 0:
                    ...
                    if g_data.red_score >= 3 or g_data.blue_score >= 3:
                        ...
                        g_data.phase = "ROUND_OVER"
                        g_data.timer = 180
            else: # [수정] FFA 승리 조건 (누군가 10점(킬) 획득)
                winner = None
                for ptid, pt in g_data.players.items():
                    if pt.get('score', 0) >= 10: winner = pt; break
                if winner:
                    g_data.phase = "MATCH_OVER"
                    g_data.msg = f"{winner.get('name', 'Someone')} WINS FFA!"
                    g_data.timer = 300
            ...
            if g_data.timer <= 0:
                if g_data.phase == "MATCH_OVER":
                    ...
                    for pid, p in list(g_data.players.items()): 
                        p['ready'] = False
                        p['last_rdy'] = server_inputs.get(pid, {}).get('a_rdy', 0)
                        p['last_t'] = server_inputs.get(pid, {}).get('a_t', 0)
                        p['last_a'] = server_inputs.get(pid, {}).get('a_a', 0)
                        p['last_d'] = server_inputs.get(pid, {}).get('a_d', 0)
                else:
                    g_data.round_num += 1
    ...
    if g_data.phase == "MENU":
        draw_txt("ONLINE MULTIPLAYER: FPS", 45, WHITE, WIDTH//2, HEIGHT//4)
        draw_txt("Your Name:", 30, GRAY, WIDTH//2, HEIGHT//2 - 50)
        pygame.draw.rect(screen, GRAY, (WIDTH//2 - 150, HEIGHT//2 - 20, 300, 40), 2)
        draw_txt(my_name + ("_" if pygame.time.get_ticks() % 1000 < 500 else ""), 30, YELLOW, WIDTH//2, HEIGHT//2)
        draw_txt("1. Create Server (Host)   |   2. Join Server (Client)", 25, WHITE, WIDTH//2, HEIGHT//2 + 70)
        ...
        draw_txt("Press ENTER to Connect / ESC to Cancel", 20, GRAY, WIDTH//2, HEIGHT//2+50)
    
    elif g_data.phase == "LOBBY":
        draw_txt("GAME LOBBY (Max 6 Players)", 50, WHITE, WIDTH//2, 40)
        draw_txt(f"MODE: {getattr(g_data, 'mode', 'TEAM')}", 30, YELLOW, WIDTH//2, 85)
        
        if getattr(g_data, 'mode', 'TEAM') == "TEAM":
            pygame.draw.line(screen, DARK_GRAY, (WIDTH//2, 120), (WIDTH//2, 500), 2)
            draw_txt("RED TEAM", 30, RED, WIDTH//4, 130)
            draw_txt("BLUE TEAM", 30, BLUE, WIDTH*3//4, 130)
            ry, by = 180, 180
            for pid, p in list(g_data.players.items()):
                cx, cy = (WIDTH//4, ry) if p.get('team')==0 else (WIDTH*3//4, by)
                if p.get('team')==0: ry += 60
                else: by += 60
                p_txt = f"{p.get('name', f'P{pid}')}" + (" (YOU)" if pid == my_id else "")
                draw_txt(p_txt, 20, WHITE, cx, cy)
                draw_txt(f"[{CHAR_DATA[p.get('c_idx', 0)]['name']}]", 16, GRAY, cx, cy+25)
                if p.get('ready'): draw_txt("READY", 20, GREEN, cx+120, cy)
        else:
            draw_txt("FREE FOR ALL", 30, ORANGE, WIDTH//2, 130)
            fy = 180
            for pid, p in list(g_data.players.items()):
                cx, cy = WIDTH//2, fy; fy += 60
                p_txt = f"{p.get('name', f'P{pid}')}" + (" (YOU)" if pid == my_id else "")
                draw_txt(p_txt, 20, WHITE, cx, cy)
                draw_txt(f"[{CHAR_DATA[p.get('c_idx', 0)]['name']}]", 16, GRAY, cx, cy+25)
                if p.get('ready'): draw_txt("READY", 20, GREEN, cx+150, cy)
            
        draw_txt("M: Toggle Mode(Host) | T: Team Swap | A/D: Change Char | SPACE: Ready", 22, YELLOW, WIDTH//2, 550)
        ...
        for pid, p in list(g_data.players.items()):
            if p.get('hp', 0) <= 0: continue
            col = WHITE if getattr(g_data, 'mode', 'TEAM') == "FFA" else (RED if p.get('team')==0 else BLUE)
            px, py = p['x']+ox, p['y']+oy
            shape = CHAR_DATA[p.get('c_idx', 0)]['shape']
            ...
            if p.get('shield', 0)>0: pygame.draw.circle(screen, CYAN, (int(px), int(py)), 22, 2)
            
            p_dir = p.get('dir', (1,0))
            ex, ey = px + p_dir[0]*20, py + p_dir[1]*20
            if p.get('use_g'): pygame.draw.circle(screen, GREEN, (int(px+p_dir[0]*15), int(py+p_dir[1]*15)), 5)
            elif shape=="sniper": 
                pygame.draw.line(screen, WHITE, (px, py), (px+p_dir[0]*30, py+p_dir[1]*30), 3)
                pygame.draw.circle(screen, GRAY, (int(px+p_dir[0]*10), int(py+p_dir[1]*10)), 4)
            ...
            draw_txt(p.get('name', f"P{pid}"), 14, WHITE, px, py - 35)
        ...
        if getattr(g_data, 'mode', 'TEAM') == "TEAM":
            ...
            # [수정] 텍스트 10킬로 변경 및 실시간 리더보드 추가
            draw_txt("FREE FOR ALL - FIRST TO 10 KILLS", 20, ORANGE, WIDTH//2, 15)
            
            # 실시간 리더보드 (우측 상단)
            sorted_ffa = sorted(g_data.players.items(), key=lambda x: x[1].get('kills', 0), reverse=True)
            lb_y = 40
            draw_txt("--- LEADERBOARD ---", 14, YELLOW, WIDTH - 120, lb_y)
            for lb_pid, lb_p in sorted_ffa:
                lb_y += 20
                lb_name = lb_p.get('name', f"P{lb_pid}")
                lb_kills = lb_p.get('kills', 0)
                draw_txt(f"{lb_name}: {lb_kills} Kills", 14, WHITE, WIDTH - 120, lb_y)

        if my_id in g_data.players:
            my_p = g_data.players[my_id]
            draw_txt(f"HP: {int(my_p.get('hp', 0))}", 20, WHITE, 20, HEIGHT-30, False)
            if my_p.get('use_g'): a_txt = f"GRENADES: {my_p.get('grenades', 0)}"; ac = ORANGE
            elif CHAR_DATA[my_p.get('c_idx', 0)]['wp']['mag']==-1: a_txt = "AMMO: INF"; ac = YELLOW
            elif my_p.get('reload', 0)>0: a_txt = "RELOADING..."; ac = RED
            else: a_txt = f"AMMO: {my_p.get('ammo', 0)} / {CHAR_DATA[my_p.get('c_idx', 0)]['wp']['mag']}"; ac = YELLOW
            draw_txt(a_txt, 20, ac, WIDTH-150, HEIGHT-30, False)
            draw_txt("Press 1(Rifle) / 2(Sniper) / 3(Knife)", 16, GRAY, WIDTH//2, HEIGHT-20)
        ...
        if g_data.phase in ["ROUND_OVER", "MATCH_OVER"]:
            over = pygame.Surface((WIDTH, HEIGHT)); over.set_alpha(180); screen.blit(over, (0,0))
            draw_txt(g_data.msg, 60, YELLOW, WIDTH//2, 100)
            
            if g_data.phase == "MATCH_OVER":
                draw_txt("--- FINAL RESULTS ---", 40, WHITE, WIDTH//2, 180)
                sorted_p = sorted(g_data.players.items(), key=lambda x: x[1].get('kills', 0), reverse=True)
                y_pos = 250
                for pid, p in sorted_p:
                    k = p.get('kills', 0)
                    d = p.get('deaths', 0)
                    dmg = p.get('dmg_dealt', 0)
                    kd = round(k / d, 2) if d > 0 else float(k)
                    p_name = p.get('name', f"P{pid}")
                    stats_txt = f"{p_name} - Kills: {k} | Deaths: {d} | K/D: {kd} | Dmg: {dmg}"
                    draw_txt(stats_txt, 25, WHITE, WIDTH//2, y_pos)
                    y_pos += 40
