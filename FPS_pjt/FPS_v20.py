...
pygame.display.set_caption("Online Tactical Shooter - 3v3 Team Battle")
...
g_data = GameState()
pickled_g_data = None
server_inputs = {} # 방장이 취합할 모든 클라이언트의 입력
...
def trigger_effect(eff):
    ...
    if len(g_data.effects) > 50: g_data.effects.pop(0) # 큐 유지 관리
...
def srv_load_map(idx):
    ...
    if idx == 0:
        ...
        g_data.obs = [[200, 200, 400, 200]] # 거대한 중앙 블록
        g_data.m_obs = []; m_obs_data = []; map_pts = [(400,100), (400,500)]
...
while True:
    ...
    # --- 서버 코어 로직 (방장 전용) ---
    if is_host:
        server_inputs[0] = my_inputs.copy() # 내 입력 저장
        my_inputs['chat'] = None # 전송 후 초기화
        ...
        pickled_g_data = pickle.dumps(g_data) # 최종 서버 상태 패키징
    ...
    if g_data.phase == "MENU":
        ...
        for pid, p in g_data.players.items():
            ...
            if pid == my_id: pygame.draw.rect(screen, YELLOW, (px-18, py-18, 36, 36), 2) # 내 캐릭터 강조
