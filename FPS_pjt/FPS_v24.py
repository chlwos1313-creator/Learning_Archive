...
pygame.display.set_caption("Online Tactical Shooter - Perfect V3")
...
CHAR_DATA = [
    ...
    {"name": "Bazooka (Hidden)", "speed": 3.0, "shape": "square", "wp": {"n": "Rocket Launcher", "cd": 80, "spd": 35, "dmg": 80, "mag": 3, "rl": 120, "type": "bazooka"}}
]
...
while True:
    ...
    # 변수 스코프 누수 완벽 방지: 렌더링용 오프셋은 ox, oy / 마우스는 mx, my 로 영구 고정
    ...
    for e in pygame.event.get():
        ...
        if e.type == pygame.KEYDOWN:
            ...
            if not is_typing:
                ...
                # [완벽 패치] 레디 버튼을 SPACE BAR 로 분리! 채팅(ENTER)과 절대 안 겹침!
    ...
    if is_host:
        ...
        if g_data.phase == "LOBBY":
            ...
            for pid, p in list(g_data.players.items()):
                ...
                # [완벽 패치] 시점(ox, oy) 변수 오염을 막기 위해 old_x, old_y, move_x, move_y로 완벽하게 격리
                old_x, old_y = p['x'], p['y']
                move_x, move_y = 0, 0
    ...
    # --- 렌더링 ---
    if g_data.phase == "MENU":
        draw_txt("SSADDEN ATTACK", 45, WHITE, WIDTH//2, HEIGHT//3)
        ...
        for pid, p in list(g_data.players.items()):
            ...
            if p.get('ready'): draw_txt("READY", 20, GREEN, cx+100, cy)
            
        # [패치 완료] 레디 키안내 변경: SPACE: Ready
        draw_txt("T: Team Swap | A/D: Change Char | SPACE: Ready", 22, YELLOW, WIDTH//2, 550)
