...
pygame.display.set_caption("Online Tactical Shooter - Hidden Weapon Patch")
...
my_socket = None
g_data = GameState()
my_inputs = {'up':0, 'down':0, 'left':0, 'right':0, 'dash':0, 'shoot':0, 'reload':0, 'swap':0, 'mx':0, 'my':0, 'c_idx':0, 'ready':False}
...
# [패치] 호스팅 취소(ESC)를 위해 서버 타임아웃 세팅 추가
def host_server():
    ...
    server.listen(1)
    server.settimeout(0.5) # 주기적으로 ESC 눌렀는지 확인
...
while True:
    ...
    # [패치] 레디 완료 시 랜덤(3번) -> 10% 바주카(4번) 또는 일반(0~2번)으로 변환
