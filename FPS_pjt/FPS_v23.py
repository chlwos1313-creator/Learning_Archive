...
pygame.display.set_caption("Online Tactical Shooter - 3v3 Team Battle (Stable)")
...
while True:
    ...
    for e in pygame.event.get():
        ...
        if e.type == pygame.KEYDOWN:
            ...
            if not is_typing:
                ...
                if e.key == pygame.K_RETURN: my_inputs['a_rdy'] += 1
    ...
    if is_host:
        ...
        # RuntimeError 방지: list() 래핑
        for pid, inp in list(server_inputs.items()):
            if inp.get('chat'):
                team_col = RED if g_data.players.get(pid, {}).get('team')==0 else BLUE
        ...
        if g_data.phase == "LOBBY":
            ...
            for pid, p in list(g_data.players.items()):
                ...
                if mx!=0 or my!=0:
                    ...
                    p['x'] += (mx/mag)*spd; p['y'] += (my/mag)*spd

                rect = pygame.Rect(p['x']-16, p['y']-16, 32, 32)
                for ob in g_data.obs + g_data.m_obs:
                    if rect.colliderect(pygame.Rect(ob)): p['x'], p['y'] = ox, oy
            ...
            if g_data.timer <= 0:
                if g_data.phase == "MATCH_OVER":
                    g_data.phase = "LOBBY"
                    # [버그 픽스] 로비로 돌아갈 때 레디 강제 해제 & 누적 엔터 입력 동기화
                    for pid, p in list(g_data.players.items()): 
                        p['ready'] = False
    ...
    if g_data.phase == "MENU":
        ...
        draw_txt("T: Team Swap | A/D: Change Char | ENTER: Ready", 22, YELLOW, WIDTH//2, 550)
