...
# [패치] 내 이름(my_name) 및 무기 스왑 키(w1, w2, w3) 추가
my_name = ""
my_inputs = {'up':0, 'down':0, 'left':0, 'right':0, 'dash':0, 'shoot':0, 'reload':0, 'swap':0, 'mx':0, 'my':0, 
             'chat':None, 'a_t':0, 'a_a':0, 'a_d':0, 'a_rdy':0, 'name':"", 'w1':0, 'w2':0, 'w3':0}
...
while True:
    ...
    # [패치] 무기 스왑 키(1, 2, 3) 및 내 이름(my_name) 업데이트
    my_inputs.update({
    ...
    if is_host:
        ...
        if g_data.phase == "LOBBY":
            ...
            for pid, p in list(g_data.players.items()):
                ...
                # [패치] 1, 2, 3 키를 이용한 실시간 무기 변경 로직
                new_wep = p['c_idx']
                if inp.get('w1'): new_wep = 0 # 라이플
                elif inp.get('w2'): new_wep = 1 # 스나이퍼
                elif inp.get('w3'): new_wep = 2 # 나이프
                
                # 바주카(4)가 아니거나, 혹은 바주카여도 다른 무기로 바꾸고 싶을 때 변경 허용
                if new_wep != p['c_idx'] and new_wep in [0, 1, 2]:
                    p['c_idx'] = new_wep
                    p['ammo'] = CHAR_DATA[new_wep]['wp']['mag']
                    p['reload'] = 0
                    p['timer'] = 20 # 무기 교체 패널티(약 0.3초)
    ...
    # --- 렌더링 ---
    if g_data.phase == "MENU":
        draw_txt("ONLINE MULTIPLAYER: 3v3 TEAM FPS", 45, WHITE, WIDTH//2, HEIGHT//4)
        ...
        draw_txt("TEAM LOBBY (Max 6 Players)", 50, WHITE, WIDTH//2, 50)
        pygame.draw.line(screen, DARK_GRAY, (WIDTH//2, 100), (WIDTH//2, 500), 2)
        draw_txt("RED TEAM", 30, RED, WIDTH//4, 110)
        draw_txt("BLUE TEAM", 30, BLUE, WIDTH*3//4, 110)
        ...
        for pid, p in list(g_data.players.items()):
            ...
            p_txt = f"{p_name}" + (" (YOU)" if pid == my_id else "")
        ...
        draw_txt("T: Team Swap | A/D: Change Starting Char | SPACE: Ready", 22, YELLOW, WIDTH//2, 550)
        ...
        for pid, p in list(g_data.players.items()):
            if p.get('hp', 0) <= 0: continue
            col = RED if p.get('team')==0 else BLUE
            px, py = p['x']+ox, p['y']+oy
