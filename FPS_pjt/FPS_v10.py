...
pygame.display.set_caption("Online Tactical Shooter - Final Patch")
...
# [패치] TCP 패킷 뭉침 현상 완벽 해결 (makefile 사용)
def host_server():
    ...
    def update(self, inp, bullets):
        ...
        if inp.get('shoot') and self.timer<=0 and self.d['reload']==0:
            if self.d['use_g'] and self.d['grenades']>0:
                # [패치] 수류탄에도 "spd" 속성을 추가하여 KeyError 방지!
...
while True:
    ...
    # [패치] 호스트가 맵 선택 창으로 돌려보냈을 때 손님의 레디 상태도 무조건 강제 초기화!
