...
pygame.display.set_caption("Online Tactical Shooter - Pro Patch")
...
# [버그 픽스] 이펙트를 생성하고 서버/클라이언트에 모두 적용하는 헬퍼 함수
def trigger_effect(eff):
    g_data.effects.append(eff)
    if is_host: # 방장의 화면에도 즉시 파티클을 소환합니다!
...
def host_server():
    ...
    # (핵심 패치) TCP 뭉침 현상 방지를 위해 소켓을 파일 스트림으로 변환
    conn_file = conn.makefile('rb') 
    
    while True:
        try:
            # recv() 대신 load()를 써서 정확히 1개의 데이터 세트만 안전하게 가져옵니다.
            p2_inputs = pickle.load(conn_file)
            conn.sendall(pickle.dumps(g_data))
            g_data.effects.clear()
        except Exception as e: 
            print("Host 네트워크 연결 끊김:", e)
            break
    ...
    def update(self, inp, bullets):
        ...
        if inp.get('shoot') and self.timer<=0 and self.d['reload']==0:
            if self.d['use_g'] and self.d['grenades']>0:
                ...
                self.timer = w['cd']
                if w['n']=="Sniper Rifle": trigger_effect(["shake", 5])
                if w['mag']!=-1:
                    self.d['ammo'] -= 1
            ...
            if g_data.phase == "MENU":
                ...
                if is_host:
                    if not g_data.p1['ready']:
                        if e.key == pygame.K_a: g_data.p1['c_idx'] = (g_data.p1['c_idx'] - 1) % 3
                        elif e.key == pygame.K_d: g_data.p1['c_idx'] = (g_data.p1['c_idx'] + 1) % 3
                        elif e.key == pygame.K_RETURN: g_data.p1['ready'] = True
                    elif e.key == pygame.K_BACKSPACE: g_data.p1['ready'] = False
                else:
                    if not client_ready:
                        if e.key == pygame.K_a: client_c_idx = (client_c_idx - 1) % 3
                        elif e.key == pygame.K_d: client_c_idx = (client_c_idx + 1) % 3
